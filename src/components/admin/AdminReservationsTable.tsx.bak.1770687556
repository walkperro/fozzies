...
export default function AdminReservationsTable({ initialRows }: { initialRows: Row[] }) {
  ...
  async function toggleArchive(r: Row) {
    ...
    setTarget((t) =>
      t && t.id === r.id ? { ...t, archived_at: next ? new Date().toISOString() : null } : t
    );
  }
  ...
  async function softDelete(r: Row) {
    ...
    setRows((prev) =>
      prev.map((x) => (x.id === r.id ? { ...x, deleted_at: new Date().toISOString() } : x))
    );
  ...
  function defaultMessageFor(k: "confirmed" | "declined" | "reschedule", r: Row) {
    if (k === "confirmed") return `We look forward to seeing you at ${r.time} on ${r.date}.`;
    if (k === "declined")
      return (
  ...
  async function sendModal() {
    ...
    // if confirmed/declined we also set status automatically
    ...
    if (kind === "reschedule") {
      ...
      if (pd || pt) {
        finalMsg += `\n\nProposed alternative: ${pd || target.date} ${pt || target.time}`;
      }
    }
  ...
  return (
    <div className="p-4 sm:p-6">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap items-center gap-2">
          {/* TIME selector (distinguishable) */}
          <select
            value={timeView}
            onChange={(e) => setTimeView(e.target.value as any)}
            className="h-9 rounded-full border border-charcoal/15 bg-cream px-3 text-xs text-charcoal outline-none"
          >
          ...
          <tbody className="text-charcoal">
            {filtered.map((r) => (
              <tr key={r.id} className="border-b border-charcoal/10 align-top">
                ...
                {/* Status selector (moved out of actions) */}
                <td className="py-3 pr-3 whitespace-nowrap">
                  <select
      ...
      {modalOpen && target && (
        <div
          className={[
            ...
            "bg-charcoal/75 backdrop-blur-[6px] bg-gradient-to-b from-charcoal/80 via-charcoal/70 to-charcoal/85",
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-2 font-serif text-2xl text-charcoal">
                    {target.name} • {target.date} {target.time}
                  </div>
